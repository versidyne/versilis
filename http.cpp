...
const char *http::head(const char *data) {
    ...
    for (i = 0; i < e; i++) {
        ...
        if (data[i] == '>' && data[i - 1] == 'd' && data[i - 2] == 'a' && data[i - 3] == 'e' && data[i - 4] == 'h' &&
            data[i - 5] == '<') {
            // scroll through data starting with marker
            for (o = i + 1; o < e; o++) {
                // detect closing head tag, set pointer, and stop scroll
                if (data[o] == '>' && data[o - 1] == 'd' && data[o - 2] == 'a' && data[o - 3] == 'e' &&
                    data[o - 4] == 'h' && data[o - 5] == '/' && data[o - 6] == '<') {
                    x -= 6;
                    break;
                }
...
const char *http::body(const char *data) {
    ...
    for (i = 0; i < e; i++) {
        // find first instance of head tag
        if (data[i] == '>' && data[i - 1] == 'y' && data[i - 2] == 'd' && data[i - 3] == 'o' && data[i - 4] == 'b' &&
            data[i - 5] == '<') {
            // scroll through data starting with marker
            for (o = i + 1; o < e; o++) {
                // detect closing head tag, set pointer, and stop scroll
                if (data[o] == '>' && data[o - 1] == 'y' && data[o - 2] == 'd' && data[o - 3] == 'o' &&
                    data[o - 4] == 'b' && data[o - 5] == '/' && data[o - 6] == '<') {
                    x -= 6;
                    break;
                }
                    // set current array cell and increment
...
const char *http::links(const char *data) {
    ...
    for (i = 0; i < e; i++) {
        // find first instance of head tag
        if (data[i] == '<' && data[i + 1] == 'a' && data[i + 2] == ' ' && data[i + 3] == 'h' && data[i + 4] == 'r' &&
            data[i + 5] == 'e' && data[i + 6] == 'f' && data[i + 7] == '=') {
            ...
            for (o = 0; o < e - 8; o++) {
                ...
                else if (link[o] == 'w' && link[o + 1] == 'w' && link[o + 2] == 'w' &&
                         link[o + 3] == '.') { /* do nothing */ }
                else { /* do nothing */ }
            }
            // find final character and set to null byte
